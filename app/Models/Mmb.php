...
class Mmb extends Model {
	...
	public static function getRows( $args )
	{
	   ...
	   $key = with(new static)->primaryKey;
		...
		$rows = array();
	    $result = DB::select( self::querySelect() . self::queryWhere() . ($where == "" ? "" : "and ($where)")."
				{$params} ". self::queryGroup() ." {$orderConditional}  {$limitConditional} ");

		$total = DB::select( "SELECT COUNT(*) AS total FROM ".$table." " . self::queryWhere().($where == "" ? "" : "and ($where)")."
	...
	static function makeInfo( $id )
	{
		...
		foreach($row as $r)
		{
			...
			$data['id']		= $r->module_id;
			$data['title'] 	= \App\Library\SiteHelpers::infoLang($r->module_title,$langs,'title');
			$data['note'] 	= \App\Library\SiteHelpers::infoLang($r->module_note,$langs,'note');
			$data['table'] 	= $r->module_db;
			$data['key'] 	= $r->module_db_key;
			$data['config'] = \App\Library\SiteHelpers::CF_decode_json($r->module_config);
			// var_dump($data['config']); exit;
			$field = array();
      if(isset($data['config']['grid'])){
        ...
        if(count($limit) >=3)
        {
            ...
            if(count($parent)>=2 )
            {
            	// $row =  DB::table($table)->where($parent[0],$parent[1])->get();
							// echo "SELECT * FROM ".$table." ".$condition ." AND ".$parent[0]." = '".$parent[1]."'"; exit;
